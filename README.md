...
| **Tests** (`tests/`) | Playwright e2e at `tests/e2e/` — two projects (`api` + `browser`), **47 tests passing** from a cold bootstrap. k6 load stubs at `tests/k6/`. | Per-service Go unit tests (scaffolds only; real tests land with feature work). Vitest suite in `core-web` has one trivial spec as a placeholder. |
