...
| **Tests** (`tests/`) | Playwright e2e at `tests/e2e/` — two projects (`api` + `browser`), **50 tests passing** from a cold bootstrap (browser project includes S1-L-01 shell smoke). k6 load stubs at `tests/k6/`. | Per-service Go unit tests (scaffolds only; real tests land with feature work). Vitest suite in `core-web` has one trivial spec as a placeholder. |
