...
| `booking.submitted` | Draft → pending_payment | booking_id | broker-svc (saga signal) |
| `booking.paid_in_full` | Final payment received | booking_id | logistics-svc (kit dispatch), finance-svc (journal) |
| `booking.cancelled` | Cancellation | booking_id, reason | broker-svc (refund flow) |
...
> Phase 1: events are notional. Cross-service signaling happens via Temporal saga signals coordinated by broker-svc.
