...
- [x] Audit log write path + `RecordAudit` gRPC — **BL-IAM-004** on 2026-04-21 (`feat/s1-e-04-iam-audit`). Producer RPC + in-process emit from `SuspendUser` inside the existing `WithTx` + scaffold `iam_grpc_adapter` on booking-svc. Also lands migration `000007_allow_audit_fk_null_cascade` — a narrowing of the append-only trigger so the FK `ON DELETE SET NULL` cascade is permitted (bug surfaced empirically during 02d dry-run, not pre-planned).
...
**Next:** all four `S1-E-04` sibling cards have landed (BL-IAM-001..004). The slice is closed; the next iam-svc work is tracked under `S1-E-06` (admin user / role / branch CRUD + login-time TOTP enforcement + the REST self-gating fidelity fix below).
...
## 2026-04-21 — BL-IAM-004 audit log RPC + first emitters

Fourth and final sibling of `S1-E-04`. Closes F1-AC acceptance ("Every state-changing call in every service produces an audit log entry") for iam-svc's own admin surface and ships the producer-side wire every future cross-service emitter will call. Promoted into the slice contract: `docs/contracts/slice-S1.md` gains a new `§ IAM` section covering `ValidateToken` + `CheckPermission` + `RecordAudit`, resolving the dangling `§ Inventory` line 531 `RecordAudit` reference. `task_codes` frontmatter gains `S1-E-04`; § Changelog appended with a 2026-04-21 row. Additive per the Bump-versi rule — no `-v2.md` bump.

- `api/grpc_api/pb/iam.proto` + regenerated `.pb.go` / `_grpc.pb.go` — new `RecordAudit` RPC. Request mirrors the `iam.audit_logs` schema 1:1 (`user_id`, `branch_id`, `resource`, `resource_id`, `action`, `old_value` / `new_value` as JSONB `bytes`, `ip`). Response returns `audit_log_id` + `created_at_unix` only — no PII echo, no state leak. `bytes` chosen over `google.protobuf.Struct` so callers marshal their own JSON and pgx / Postgres still validates at insert time.
- `api/grpc_api/server.go` — new `RecordAudit` handler mirroring `ValidateToken`'s pattern (tracer span, `logging.LogWithTrace`, `status.Error(apperrors.GRPCCode, apperrors.GRPCMessage)` — same sanitised message body the BL-IAM-003 ride-along fix introduced).
- `service/audit.go` (new) + `service/service.go` — `RecordAudit(ctx, params) (*RecordAuditResult, error)`. Validates `resource` + `action` non-empty, parses optional UUID / IP strings (empty → NULL in storage; non-empty → parsed or `ErrValidation`), passes JSONB bytes through unmodified, calls `s.store.InsertAuditLog`. No transaction — single statement. No new sqlc query: `InsertAuditLog` was already generated by BL-IAM-001/003 scaffolding against migration 000003's schema.
- `service/admin.go` — `SuspendUser`'s existing `WithTx` closure now includes a single `q.InsertAuditLog(...)` call between the status flip and the session revoke. Business-success ↔ audit-success are atomic: a failed audit insert rolls back the status flip too. Actor UUID comes from the verified bearer payload (captured as `ActorUserID` in BL-IAM-003, previously `_ = actorUUID`); target branch inherited from the target user row pre-flip; `old_value` captures the pre-flip status (`active` on first suspend, `suspended` on re-suspend — every admin action auditable), `new_value` is always `{"status":"suspended"}`.
- `service/audit_test.go` (new) — 9 unit cases: nil params / missing resource / missing action / malformed actor UUID / malformed branch UUID / malformed IP / empty-optional-fields-map-to-null / populated-fields-forwarded-to-store / store-error-wraps-as-ErrInternal. `internal/mocks/istore.go` gets an `InsertAuditLog` override. The `SuspendUser` tx body continues to be exercised end-to-end in the e2e (02c for the business flow; 02d for the audit row shape), matching the `Test_Logout_*` pattern of "pre-tx guards in unit tests, tx body in e2e".

Consumer-side scaffold: `services/booking-svc/adapter/iam_grpc_adapter/` (new — mirrors finance-svc: `adapter.go` + `validate_token.go` + `check_permission.go` + `record_audit.go` + vendored slim `pb/iam.proto` carrying only the three RPCs booking-svc calls). `cmd/start.go` dials `config.Iam.GrpcTarget` (default `iam-svc:50051`) with `insecure.NewCredentials()` + `otelgrpc` stats handler. Adapter passed into `NewService` as an `IamClient` interface so the first real consumer (S1-E-03 draft booking create) is a one-line change at the call site. No booking-svc service method consumes it yet — booking-svc has zero state-changing endpoints today.

Migration 000007 surfaced empirically when 02d's first run populated `iam.audit_logs` and the subsequent `migrate-down` tripped `insufficient_privilege`: migration 000004's down tried to delete the admin fixture user, the FK `ON DELETE SET NULL` on `audit_logs.user_id` fired a cascade UPDATE, and the blanket-reject trigger refused it. Root cause is pre-existing (migration 000003) but only observable once a real write path into `audit_logs` existed. The new `000007_allow_audit_fk_null_cascade.{up,down}.sql` narrows the UPDATE branch to permit exclusively-null-FK-column UPDATEs (all other columns verified byte-identical via `=` + `IS NOT DISTINCT FROM`; at least one of `user_id` / `branch_id` must transition non-null → null). Tamper evidence preserved (resource / action / old_value / new_value / ip / created_at frozen); compliance-driven user deletion (UU PDP right-to-erasure) cascades cleanly. Down migration `TRUNCATE`s the table before restoring the strict body — TRUNCATE bypasses per-row triggers by design, so reverting 000007 semantically returns to "no audit writes yet".

F1 backend notes line 157 correction: the earlier "fire-and-forget to a buffered channel, with a flush-on-shutdown guarantee" plan is explicitly superseded by BL-IAM-004's synchronous+atomic model (in-process via `WithTx`, cross-service via synchronous RPC). Rationale pinned in the plan: the compliance invariant F1-AC asks for is easier to hold under strict atomicity, `SuspendUser`'s existing `WithTx` absorbs the emit as one line per the BL-IAM-003 promise, and the append-only trigger + Serializable isolation + `tx.go`'s retry-on-40001 are the right tier for a compliance log. Q008 (UU PDP) turned out to be about retention / consent / DSR / DPO, not audit atomicity, so no Q-file edit was needed.

e2e coverage: `tests/e2e/tests/02d-iam-svc-audit.spec.ts`, seven cases run serially: (1) baseline count — capture existing suspend-audit count for the sacrifice target so subsequent cases assert deltas and the spec is re-run-safe without a DB reset, (2) admin suspends target → exactly one new audit row with `resource='user'`, `action='suspend'`, actor=admin UUID, target=resource_id, `new_value.status='suspended'`, (3) re-suspend writes a second audit row (every admin action auditable, including no-op status flips), (4) UPDATE rejected with errcode 42501 `insufficient_privilege`, (5) DELETE rejected same, (6) narrowed trigger allows `UPDATE user_id = NULL` (the exact cascade shape) — pins migration 000007's permit path without deleting the admin fixture, (7) narrowed trigger still rejects mixed FK+non-FK UPDATE (`user_id = NULL, action = 'tampered'` → 42501). Adds `tests/e2e/lib/pg-client.ts` — first time the e2e harness asserts DB state directly; previously HTTP-only. Introduces `pg@^8.13.1` + `@types/pg` as devDeps. 02a/b/c regression stays at 24/24 (10 + 4 + 10) for a total of 31/31 across the iam suite.

Security review: one pass, zero findings at confidence ≥ 8 — forged `user_id` on `RecordAudit` (documented trust model, gRPC port docker-network-internal, no auth interceptor — unchanged by this branch), JSONB passthrough (Postgres validates on INSERT, crash-safe), trigger bypass in migration 000007 (walked every UPDATE shape, guard holds), information leak (same sanitised-message pattern as BL-IAM-003, no new leak surface), insecure gRPC dial (documented convention), `%q` JSON construction in `admin.go` (bounded enum input — no injection), test-only code excluded per FP rules.

Direct-RPC e2e testing of `RecordAudit` was deferred to the first real cross-service consumer (S1-E-03 draft booking create) rather than bolting a Go test client into `tests/e2e/` now. The service layer unit tests cover the code paths; the RPC surface is type-checked by protoc + registered at iam-svc startup; the first consumer will exercise the wire naturally.
