...
| 1     | S1-E-08     | Monitoring migration — register `grpc.health.v1.Health` on all 10 backends; install `grpc_health_probe` in Dockerfiles; wire docker-compose healthchecks (gateway keeps REST healthcheck). Metrics split out — `/metrics` stays on REST port until each service's REST package is removed per `BL-REFACTOR-*` | MH-MVP | 143 | S1 | S1-E-08 | BL-MON-001 | E | todo | BL-JNT-001..004; BL-EGV-001; BL-LGV-001 | All 10 backends respond SERVING to `grpc_health_probe`; `docker compose up` brings the stack healthy; gateway REST healthcheck green |
| 1     | F1-W7       | Edge auth at `gateway-svc` — `iam_grpc_adapter` + Bearer middleware + fail-closed 502 on `iam-svc` unreachable | MH-MVP | 144 | S1 | S1-E-09 | BL-GTW-001 | E | todo | BL-JNT-001..004; BL-EGV-001; BL-LGV-001 | Valid bearer → 200; invalid → 401; `iam-svc` down → 502; trace shows gateway→iam `ValidateToken` span |
| 1     | F2-W2       | Gateway `catalog_grpc_adapter` + REST routes for `/v1/packages*` and `/v1/package-departures/{id}`; e2e migrates to `gateway-svc:4000` | MH-MVP | 145 | S1 | S1-E-10 | BL-GTW-002 | E | todo | BL-JNT-001..004; BL-EGV-001; BL-LGV-001; BL-MON-001; BL-GTW-001 | Public catalog reads succeed through gateway; `02e-catalog-svc-read.spec.ts` passes against `gateway-svc:4000` |
| 1     | S1-E-11     | Remove `catalog-svc` REST package (ADR 0009); migrate `/system/diagnostics/db-tx` to a gRPC method | MH-MVP | 146 | S1 | S1-E-11 | BL-REFACTOR-001 | E | todo | BL-JNT-001..004; BL-EGV-001; BL-LGV-001; BL-GTW-002 | `services/catalog-svc/api/rest_oapi/` is gone; gateway-routed e2e still green; catalog runs gRPC-only |
| 1     | S1-E-12     | Move iam client-facing REST (`/v1/sessions*`, `/v1/me*`, `/v1/users*`) to `gateway-svc`; add matching iam gRPC RPCs | MH-MVP | 147 | S1 | S1-E-12 | BL-IAM-018 | E | todo | BL-JNT-001..004; BL-EGV-001; BL-LGV-001; BL-MON-001; BL-GTW-001 | Login/logout/refresh/me flow through `gateway-svc:4000`; `services/iam-svc/api/rest_oapi/` removed |
