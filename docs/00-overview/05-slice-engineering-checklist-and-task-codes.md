...
| S1-E-08 | E | **Monitoring migration (ADR 0009 — health only; metrics deferred)** — register `grpc.health.v1.Health` for empty-name + service-name on all 10 backends; install `grpc_health_probe` in backend Dockerfiles; wire `docker-compose.dev.yml` healthchecks across all 11 services (gateway uses REST `wget --spider`; backends use `grpc_health_probe`); gateway `depends_on` uses `condition: service_healthy`. Metrics migration is a later card — `/metrics` stays on each service's REST port until its `BL-REFACTOR-*` lands | S1-J-01 |
...
| S1-E-13 | E | **Scaffold-only REST sweep (ADR 0009)** — delete `api/rest_oapi/` from the 7 pure-scaffold backends (booking, crm, jamaah, logistics, ops, payment, visa); rewrite each service's `util/monitoring/` to OTLP metrics push (catalog-svc G7 pattern); drop `util/monitoring/panic.go`; retire the 7 matching gateway REST adapters + `/v1/<svc>/system/live` routes + `external.<svc>_svc.address` config fields; trim compose + prometheus scrape config | S1-E-11 |
| S1-E-14 | E | **finance-svc ADR 0009 realignment** — delete finance REST + local `bearer_auth` middleware + local `iam_grpc_adapter`; add `FinancePing` gRPC RPC on finance-svc + gateway `finance_grpc_adapter`; move `/v1/finance/ping` + `/v1/finance/system/live` to gateway REST; add gateway `RequirePermission` middleware calling `iam.CheckPermission`; migrate `02b`/`02c` e2e specs to `gateway-svc:4000` | S1-E-12, S1-E-13 |
