// Package jamaah_rest_adapter is gateway-svc's REST client into jamaah-svc.
//
...
// Adapter wraps an HTTP client targeting jamaah-svc.
type Adapter struct {
...
// NewAdapter creates a REST adapter for jamaah-svc.
//
// The HTTP client is wrapped with otelhttp.NewTransport so the trace context
// from the inbound gateway request propagates over the wire to jamaah-svc as a
// W3C Trace Context header. jamaah-svc's Fiber middleware picks it up and
