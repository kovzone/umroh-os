...
// LivenessResult mirrors jamaah-svc's /system/live response envelope.
type LivenessResult struct {
	OK bool `json:"ok"`
}

// envelope is jamaah-svc's outer JSON shape: {"data": <T>}.
type livenessEnvelope struct {
	Data LivenessResult `json:"data"`
}

// GetSystemLive calls jamaah-svc's GET /system/live and returns the decoded result.
//
...
func (a *Adapter) GetSystemLive(ctx context.Context) (*LivenessResult, error) {
	const op = "jamaah_rest_adapter.GetSystemLive"
	...
	logger.Info().Str("op", op).Str("url", url).Msg("calling jamaah-svc")
	...
	if resp.StatusCode != http.StatusOK {
		err = fmt.Errorf("jamaah-svc returned status %d", resp.StatusCode)
		logger.Error().Err(err).Msg("")
		span.RecordError(err)
		span.SetStatus(codes.Error, err.Error())
		return nil, err
