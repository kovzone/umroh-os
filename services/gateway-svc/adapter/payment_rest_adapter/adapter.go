// Package payment_rest_adapter is gateway-svc's REST client into payment-svc.
//
...
// Adapter wraps an HTTP client targeting payment-svc.
type Adapter struct {
...
// NewAdapter creates a REST adapter for payment-svc.
//
// The HTTP client is wrapped with otelhttp.NewTransport so the trace context
// from the inbound gateway request propagates over the wire to payment-svc as a
// W3C Trace Context header. payment-svc's Fiber middleware picks it up and
