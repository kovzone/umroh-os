// Package finance_rest_adapter is gateway-svc's REST client into finance-svc.
//
...
// Adapter wraps an HTTP client targeting finance-svc.
type Adapter struct {
...
// NewAdapter creates a REST adapter for finance-svc.
//
// The HTTP client is wrapped with otelhttp.NewTransport so the trace context
// from the inbound gateway request propagates over the wire to finance-svc as a
// W3C Trace Context header. finance-svc's Fiber middleware picks it up and
