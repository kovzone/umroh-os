// Package catalog_rest_adapter is gateway-svc's REST client into catalog-svc.
//
...
// Adapter wraps an HTTP client targeting catalog-svc.
type Adapter struct {
...
// NewAdapter creates a REST adapter for catalog-svc.
//
// The HTTP client is wrapped with otelhttp.NewTransport so the trace context
// from the inbound gateway request propagates over the wire to catalog-svc as a
// W3C Trace Context header. catalog-svc's Fiber middleware picks it up and
