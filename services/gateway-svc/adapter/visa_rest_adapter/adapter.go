// Package visa_rest_adapter is gateway-svc's REST client into visa-svc.
//
...
// Adapter wraps an HTTP client targeting visa-svc.
type Adapter struct {
...
// NewAdapter creates a REST adapter for visa-svc.
//
// The HTTP client is wrapped with otelhttp.NewTransport so the trace context
// from the inbound gateway request propagates over the wire to visa-svc as a
// W3C Trace Context header. visa-svc's Fiber middleware picks it up and
