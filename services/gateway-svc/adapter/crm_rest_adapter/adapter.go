// Package crm_rest_adapter is gateway-svc's REST client into crm-svc.
//
...
// Adapter wraps an HTTP client targeting crm-svc.
type Adapter struct {
...
// NewAdapter creates a REST adapter for crm-svc.
//
// The HTTP client is wrapped with otelhttp.NewTransport so the trace context
// from the inbound gateway request propagates over the wire to crm-svc as a
// W3C Trace Context header. crm-svc's Fiber middleware picks it up and
