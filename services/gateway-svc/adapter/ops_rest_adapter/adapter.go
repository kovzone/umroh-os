// Package ops_rest_adapter is gateway-svc's REST client into ops-svc.
//
...
// Adapter wraps an HTTP client targeting ops-svc.
type Adapter struct {
...
// NewAdapter creates a REST adapter for ops-svc.
//
// The HTTP client is wrapped with otelhttp.NewTransport so the trace context
// from the inbound gateway request propagates over the wire to ops-svc as a
// W3C Trace Context header. ops-svc's Fiber middleware picks it up and
