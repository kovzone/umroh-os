...
import (
	...
	"crm-svc/api/grpc_api"
	"crm-svc/api/grpc_api/pb"
	"crm-svc/api/rest_oapi"
	"crm-svc/api/rest_oapi/middleware"
	"crm-svc/util/monitoring"
...
// crm-svc pilot scaffold exposes only the three standard scaffold endpoints:
//
//   - GET /system/live
//   - GET /system/ready
...
func runRestServer(port int, api rest_oapi.ServerInterface, metricsEnabled bool, serviceName string) {
	...
	// to scan (e.g. "crm-svc GET /system/diagnostics/db-tx").
...
// runGrpcServer runs the gRPC server exposing the CrmService surface plus the
...
func runGrpcServer(address string, apiServer *grpc_api.Server) *grpc.Server {
	...
	pb.RegisterCrmServiceServer(grpcServer, apiServer)
