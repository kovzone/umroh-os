...
import (
	...
	"payment-svc/api/rest_oapi"
	"payment-svc/api/rest_oapi/middleware"
...
// payment-svc pilot scaffold exposes only the three standard scaffold endpoints:
//
//   - GET /system/live
//   - GET /system/ready
...
func runRestServer(port int, api rest_oapi.ServerInterface, metricsEnabled bool, serviceName string) {
	...
	// to scan (e.g. "payment-svc GET /system/diagnostics/db-tx").
...
// runGrpcServer runs the gRPC server exposing the PaymentService surface plus the
