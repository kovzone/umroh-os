...
//
// gRPC codes are translated back to apperrors sentinels so the Fiber error
// middleware renders the same envelope as native finance-svc errors. Unauthenticated
// → ErrUnauthorized, PermissionDenied → ErrForbidden, anything else → ErrInternal
...
// sentinel the finance-svc error middleware can render. The default branch is
