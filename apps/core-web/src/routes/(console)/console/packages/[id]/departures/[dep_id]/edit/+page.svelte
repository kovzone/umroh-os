<script lang="ts">
  ...
  const dep = $derived(data.departure);

  const currentDepartureDate = $derived(form?.values?.departure_date ?? dep?.departure_date ?? '');
  const currentReturnDate = $derived(form?.values?.return_date ?? dep?.return_date ?? '');
  const currentSeats = $derived(form?.values?.total_seats ?? String(dep?.total_seats ?? ''));
  const currentStatus = $derived(form?.values?.status ?? dep?.status ?? 'draft');
...
<main class="page-shell">
  <header class="topbar">
    ...
    <span class="topbar-sep">/</span>
    <span class="topbar-current">Edit Departure</span>
  </header>
  ...
  <section class="canvas">
    <div class="page-head">
      <div>
        ...
        <p>
          {#if data.packageName}Paket: <strong>{data.packageName}</strong> &nbsp;·&nbsp;{/if}
          ID: <code>{dep?.id}</code>
        </p>
      </div>
    </div>
