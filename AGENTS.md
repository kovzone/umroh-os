...
7. **Contribution workflow (how to ship changes)**
   - `CONTRIBUTING.md` — branch/PR workflow, minimum quality gate, shared-vs-local config rules, and optional GitHub compare URL patterns (see § Pre-filled PR open links if you want title/body in the URL).
...
- **After `git push` (handoff to humans):** Provide a **copy-paste ready** PR description in markdown: same sections and headings as `.github/pull_request_template.md`, fully filled for that PR, in a fenced code block or plain block so it can be pasted into GitHub in one step. A normal compare link (`dev`…`head`) is enough for navigation; encoding the whole body in a URL is optional (see `CONTRIBUTING.md` § Pre-filled PR open links if you use that flow).
